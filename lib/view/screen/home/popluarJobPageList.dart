...
class JobListPopular extends StatelessWidget {
  const JobListPopular({super.key});

  @override
  Widget build(BuildContext context) {
    return Consumer<JobsNotifier>(builder: (context, jobNotifer, child) {
      // getting all the jobs
      //  uploaded tile is the single element
      jobNotifer.getAllJobs();
      return SizedBox(
        // <We have to put that parameter on which my UI getting dependent here we are waiting for JobList  >
        child: FutureBuilder<List<Job>>(
            future: jobNotifer.jobList,
